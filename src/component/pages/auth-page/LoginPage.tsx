            ...
            <div className="relative z-10 flex items-center justify-center min-h-screen px-4">
                <div className="relative flex flex-col justify-around items-center text-white w-full min-h-screen" >
                    <div className="w-full max-w-sm rounded-3xl bg-white/10 backdrop-blur-md p-8 shadow-2xl mx-5">
                        <div >
                            <h1 className="flex flex-col text-3xl font-sans">Login <span className="text-sm font-thin mt-2">Teman Wisata Pintar</span></h1>
                        </div>
                            <div className="font-poppins ">
                                {message && (
                                    <div className="border border-green-500 bg-green-800 opacity-20 rounded-xl">
                                        <p className="text-xl text-white">{message}</p>
                                    </div>
                                )}
                                {error && (
                                    <div className="border border-red-500 bg-red-800 opacity-20 rounded-xl">
                                        <p className="text-sm text-white p-3">{error}</p>
                                    </div>
                                )}
                            </div>
                            <form onSubmit={handleSubmit} className="flex flex-col gap-4  py-5 font-poppins ">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full border border-gray-400 rounded-xl outline-none x-2 py-2"/>
                                </div>
                                    <p>create account, <span className="text-cyan-800 hover:text-cyan-400 transition-all duration-300 delay-75"><a href="/register">new account</a></span></p>
                                    <button type="submit" className="border border-blue-800 rounded-xl hover:bg-blue-800 hover:text-white transition-all duration-300 delay-75">login</button>
                            </form>
                    </div>
                </div>
            </div>
